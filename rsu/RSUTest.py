...
from indy import anoncreds, did, ledger, pool, wallet, blob_storage, crypto
...
class RSUTest:
    ...
    async def prover_seacrh_credentials_to_client(self, proof_req_from_client):
        logger.info("\"RSU\" -> Get credentials for \"Toll-Application\" Proof Request")
        search_for_toll_application_proof_request = \
        await anoncreds.prover_search_credentials_for_proof_req(self.rsu['wallet'],
                                                                proof_req_from_client, None)
        ...
        self.rsu['toll_application_proof'] = \
            await anoncreds.prover_create_proof(self.rsu['wallet'], proof_req_from_client,
                                                self.rsu['toll_application_requested_creds'], self.rsu['master_secret_id'],
                                                self.rsu['schemas_for_toll_application'],
                                                self.rsu['cred_defs_for_toll_application'],
                                                self.rsu['revoc_states_for_toll_application'])
    ...
    async def encrypt_message(self, response, client_verkey):
        ...
        """
        #self.encrypted_response = await crypto.auth_crypt(self.rsu['wallet'], self.rsu['key'], client_verkey, response)
        self.encrypted_response = await crypto.pack_message(self.rsu['wallet'], response, client_verkey, self.rsu['key'])
    ...
    async def decrypt_message(self, response):
        ...
        """
        # decrypted_response = await crypto.auth_decrypt(self.rsu['wallet'], client_verkey, response)
        decrypted_response = await crypto.unpack_message(self.rsu['wallet'], response)
    ...
    async def decrypt2_message(self, response):
        ...
        self.print_log("OK", str(self.rsu['wallet']))
        self.print_log("OK", str(isinstance(self.rsu['wallet'], int)))
        self.print_log("OK", str(self.rsu['key']))
        self.print_log("OK", str(isinstance(self.rsu['key'], str)))
        decrypted_response = await crypto.auth_decrypt(self.rsu['wallet'], self.rsu['key'], response)
        return decrypted_response
    ...
    #     get_cred_def_request = await ledger.build_get_cred_def_request(self.rsu['did'], cred_def)
    #     get_cred_def_response =\
    ...
    #     try:
    #         #(transcript_cred_request, transcript_cred_request_metadata) = await anoncreds.prover_create_credential_req(self.rsu['wallet'], self.rsu['did'], message, json.dumps(transcript_cred_def), self.master_secret)
    #         # print("DEBUG", self.rsu['wallet'])
    #         # print("DEBUG", self.rsu['did'])
    ...
    #         # print("OK", self.rsu['master_secret_id'])
    #         cred_offer_tmp = cred_offer.replace("'", "\"")
    #         cred_offer_json =  json.loads(cred_offer_tmp)
    ...
    #         # (transcript_cred_request, transcript_cred_request_metadata) = await anoncreds.prover_create_credential_req(self.rsu['wallet'], self.rsu['did'], json.dumps(cred_offer), rsu_transcript_cred_def, self.rsu['master_secret_id'])
    #         (transcript_cred_request, transcript_cred_request_metadata) = await anoncreds.prover_create_credential_req(self.rsu['wallet'], self.rsu['did'], json.dumps(cred_offer_json), rsu_transcript_cred_def, self.rsu['master_secret_id'])
    ...
    async def get_credential_definition(self, cred_def_id):
        try:
            # get_cred_def_request = await ledger.build_get_cred_def_request(self.rsu['did'], str(cred_def_id))
            get_cred_def_request = await ledger.build_get_cred_def_request(self.rsu['did'], cred_def_id)
    ...
    #                 pass
    #         self.rsu['wallet'] = await wallet.open_wallet(wallet_config, wallet_credentials)
