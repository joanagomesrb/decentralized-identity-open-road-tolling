...
async def run():
    ...
    serverAddress =("192.168.94.108", 5558)
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.connect(serverAddress)
        tmp = json.dumps(transcript_cred_offer_object)
        s.send(tmp.encode("utf-8"))
        raw_data = s.recv(8192)
        print("received cred request from rsu")
        print(raw_data)
        data = raw_data.decode("utf-8")
        dataj = json.loads(data)
        rsu['transcript_cred_request'] = dataj["cred_req"]
        rsu['transcript_cred_request_metadata'] = dataj["cred_req_metadata"]
    ...
    # this is what the toll company has to receive
    ...
    # send toll_application_request_proof to rsu
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.connect(serverAddress)
        tmp2 = tollcompany['transcript_cred']
        sendj = {"type": "transcript_cred", "transcript": tmp2}
        sends = json.dumps(sendj)
        s.send(sends.encode("utf-8"))

        # raw_data2 = s.recv(8192)
        # print("received toll application proof from rsu")
        # print(raw_data2)
        # data = raw_data.decode("utf-8")
        # dataj = json.loads(data)
        # rsu['toll_application_proof'] = dataj
    ...
    logger.info("==============================")
    logger.info("== Apply for the rsu (toll collector) with Toll Company - Transcript proving ==")
    logger.info("------------------------------")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.connect(serverAddress)
        tmp2 = tollcompany['toll_application_proof_request']
        sendj = {"type": "toll_application_proof_req", "proof_req": tmp2}
        sends = json.dumps(sendj)
        s.send(sends.encode("utf-8"))
        ...
        while receiving:
            ...
            if len(full_msg)-HEADERSIZE == l:
                ...
                new_msg = True
                receiving = False
        # raw_data2 = s.recv(32768)
        print("received toll application proof from rsu")
        print(raw_data2)
        # print(raw_data2)
        # print(len(raw_data2))
        # data = raw_data2.decode("utf-8")
        # dataj = json.loads(data)
        rsu['toll_application_proof'] = raw_data2

    # logger.info("\"Toll Company\" -> Send \"Toll-Application\" Proof Request to RSU")
    # rsu['toll_application_proof_request'] = tollcompany['toll_application_proof_request']
    ...
    print("tollcompany['toll_application_proof']")
    print(tollcompany['toll_application_proof'])
    toll_application_proof_object = json.loads(tollcompany['toll_application_proof'])
    # toll_application_proof_object = tollcompany['toll_application_proof']
    ...
    assert 'enabled' == \
           toll_application_proof_object['requested_proof']['revealed_attrs']['attr2_referent']['raw']
    # assert 'graduated' == \
    #        toll_application_proof_object['requested_proof']['revealed_attrs']['attr4_referent']['raw']
    # assert '123-45-6789' == \
    #        toll_application_proof_object['requested_proof']['revealed_attrs']['attr5_referent']['raw']

    assert 'rsu' == toll_application_proof_object['requested_proof']['self_attested_attrs']['attr1_referent']
    # assert 'Garcia' == toll_application_proof_object['requested_proof']['self_attested_attrs']['attr2_referent']
    # assert '123-45-6789' == toll_application_proof_object['requested_proof']['self_attested_attrs']['attr6_referent']

    assert await anoncreds.verifier_verify_proof(tollcompany['toll_application_proof_request'], tollcompany['toll_application_proof'],
                                                 tollcompany['schemas_for_toll_application'],
                                                 tollcompany['cred_defs_for_toll_application'],
                                                 tollcompany['revoc_ref_defs_for_toll_application'],
                                                 tollcompany['revoc_regs_for_toll_application'])

    # logger.info("==============================")
    # logger.info("== Apply for the toll with Acme - Getting Job-Certificate Credential ==")
    # logger.info("------------------------------")
